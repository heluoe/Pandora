...
#define _PINOCCHIO_CHECK_INPUT_ARGUMENT_1(condition) \
    _PINOCCHIO_CHECK_INPUT_ARGUMENT_2(condition, "The following check on the input argument has failed: " #condition)

#define _PINOCCHIO_CHECK_INPUT_ARGUMENT_0

/// \brief Macro to check an assert-like condition and throw a std::invalid_argument exception (with a message) if
/// violated.
#define PINOCCHIO_CHECK_INPUT_ARGUMENT(...)                                                 \
    _PINOCCHIO_EXPAND(_PINOCCHIO_EXPAND(                                                    \
        _PINOCCHIO_GET_OVERRIDE_FOR_CHECK_INPUT_ARGUMENT(__VA_ARGS__,                       \
...
#define PINOCCHIO_CHECK_ARGUMENT_SIZE(...)                                                \
    _PINOCCHIO_EXPAND(_PINOCCHIO_EXPAND(                                                  \
        _PINOCCHIO_GET_OVERRIDE_FOR_CHECK_ARGUMENT_SIZE(__VA_ARGS__,                      \
                                                        _PINOCCHIO_CHECK_ARGUMENT_SIZE_3, \
