...
typename D1::Scalar angleBetweenQuaternions(const Eigen::QuaternionBase<D1>& q1, const Eigen::QuaternionBase<D2>& q2)
{
    ...
    theta = internal::if_then_else(internal::LT, innerprod, Scalar(0), PI_value - theta, theta);
    return theta;
}
...
void firstOrderNormalize(const Eigen::QuaternionBase<D>& q)
{
    ...
    assert(static_leq::op(math::fabs(static_cast<Scalar>(q.norm() - Scalar(1))),
                          math::max(M * sqrt(N2) * (N2 - Scalar(1)) * (N2 - Scalar(1)) / Scalar(2),
...
struct quaternionbase_assign_impl_if_t_negative
{
    ...
    static inline void run(Scalar t, Eigen::QuaternionBase<QuaternionDerived>& q, const Matrix3& mat)
    {
