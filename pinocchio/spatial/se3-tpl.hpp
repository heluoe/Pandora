...
struct SE3Tpl : public SE3Base<SE3Tpl<_Scalar, _Options> >
{
    ...
    SE3Tpl(const Eigen::MatrixBase<Matrix3Like>& R, const Eigen::MatrixBase<Vector3Like>& trans) : rot(R), trans(trans)
    {
        EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(Vector3Like, 3)
    ...
    SE3Tpl(int) : rot(AngularType::Identity()), trans(LinearType::Zero()) {}

    template <int O2>
    SE3Tpl(const SE3Tpl<Scalar, O2>& clone) : rot(clone.rotation()), trans(clone.translation())
    {
    }
    ...
    static SE3Tpl Identity() { return SE3Tpl(1); }
    ...
    /// aXb = bXa.inverse()
    SE3Tpl inverse() const { return SE3Tpl(rot.transpose(), -rot.transpose() * trans); }

    static SE3Tpl Random() { return SE3Tpl().setRandom(); }
    ...
    ActionMatrixType toActionMatrixInverse_impl() const
    {
        typedef Eigen::Block<ActionMatrixType, 3, 3> Block3;
        ActionMatrixType M;
        M.template block<3, 3>(ANGULAR, ANGULAR) = M.template block<3, 3>(LINEAR, LINEAR) = rot.transpose();
...
#undef PINOCCHIO_INTERNAL_COMPUTATION
    ...
    Vector3 act_impl(const Vector3& p) const { return Vector3(rotation() * p + translation()); }

    Vector3 actInv_impl(const Vector3& p) const { return Vector3(rotation().transpose() * (p - translation())); }
    ...
    {
        return rotation().isApprox(m2.rotation(), prec) && translation().isApprox(m2.translation(), prec);
    }
    ...
    void normalize() { rot = orthogonalProjection(rot); }
    ...
    ///
    /// \note This is similar to the SLERP operation which acts initially for rotation but applied here to rigid
    /// transformation.
    ///
