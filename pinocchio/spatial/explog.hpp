...
typename Eigen::Matrix<typename Vector3Like::Scalar, 3, 3, PINOCCHIO_EIGEN_PLAIN_TYPE(Vector3Like)::Options> exp3(
    const Eigen::MatrixBase<Vector3Like>& v)
{
    ...
    const Scalar alpha_vxvx = internal::if_then_else(internal::GT,
                                                     t,
                                                     TaylorSeriesExpansion<Scalar>::template precision<3>(),
                                                     (1 - ct) / t2,
                                                     Scalar(1) / Scalar(2) - t2 / 24);
    const Scalar alpha_vx = internal::if_then_else(
        internal::GT, t, TaylorSeriesExpansion<Scalar>::template precision<3>(), (st) / t, Scalar(1) - t2 / 6);
    ...
    ct = internal::if_then_else(
        internal::GT, t, TaylorSeriesExpansion<Scalar>::template precision<3>(), ct, Scalar(1) - t2 / 2);
...
Eigen::Matrix<typename Matrix3Like::Scalar, 3, 1, PINOCCHIO_EIGEN_PLAIN_TYPE(Matrix3Like)::Options> log3(
...
void Jexp3(const Eigen::MatrixBase<Vector3Like>& r, const Eigen::MatrixBase<Matrix3Like>& Jexp)
{
    ...
    const Scalar a = internal::if_then_else(internal::LT,
                                            n,
                                            TaylorSeriesExpansion<Scalar>::template precision<3>(),
                                            Scalar(1) - n2 / Scalar(6),
                                            sn * n_inv);
    const Scalar b = internal::if_then_else(internal::LT,
                                            n,
                                            TaylorSeriesExpansion<Scalar>::template precision<3>(),
                                            -Scalar(1) / Scalar(2) - n2 / Scalar(24),
                                            -(1 - cn) * n2_inv);
    const Scalar c = internal::if_then_else(internal::LT,
                                            n,
                                            TaylorSeriesExpansion<Scalar>::template precision<3>(),
...
SE3Tpl<typename MotionDerived::Scalar, PINOCCHIO_EIGEN_PLAIN_TYPE(typename MotionDerived::Vector3)::Options> exp6(
    const MotionDense<MotionDerived>& nu)
{
    ...
    alpha_wxv = internal::if_then_else(internal::LT,
                                       t,
                                       TaylorSeriesExpansion<Scalar>::template precision<3>(),
                                       Scalar(1) / Scalar(2) - t2 / 24,
                                       (Scalar(1) - ct) * inv_t2);
    ...
    alpha_v = internal::if_then_else(
        internal::LT, t, TaylorSeriesExpansion<Scalar>::template precision<3>(), Scalar(1) - t2 / 6, (st) / t);

    alpha_w = internal::if_then_else(internal::LT,
                                     t,
                                     TaylorSeriesExpansion<Scalar>::template precision<3>(),
    ...
    diagonal_term = internal::if_then_else(
        internal::LT, t, TaylorSeriesExpansion<Scalar>::template precision<3>(), Scalar(1) - t2 / 2, ct);
...
SE3Tpl<typename Vector6Like::Scalar, PINOCCHIO_EIGEN_PLAIN_TYPE(Vector6Like)::Options> exp6(
...
MotionTpl<typename Matrix4Like::Scalar, Eigen::internal::traits<Matrix4Like>::Options> log6(
...
void Jexp6(const MotionDense<MotionDerived>& nu, const Eigen::MatrixBase<Matrix6Like>& Jexp)
{
    ...
    const Scalar beta = internal::if_then_else(internal::LT,
                                               t,
                                               TaylorSeriesExpansion<Scalar>::template precision<3>(),
                                               Scalar(1) / Scalar(12) + t2 / Scalar(720),
                                               t2inv - st * tinv * inv_2_2ct);

    const Scalar beta_dot_over_theta =
        internal::if_then_else(internal::LT,
                               t,
                               TaylorSeriesExpansion<Scalar>::template precision<3>(),
                               Scalar(1) / Scalar(360),
    ...
    switch (op)
    {
        case SETTO: {
            ...
            const Matrix3 J(alphaSkew(.5, p) + (beta_dot_over_theta * wTp) * w * w.transpose() -
...
SE3Tpl<Scalar, Options> SE3Tpl<Scalar, Options>::Interpolate(const SE3Tpl& A, const SE3Tpl& B, const OtherScalar& alpha)
{
