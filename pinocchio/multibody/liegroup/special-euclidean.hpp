    ...
    : public LieGroupBase<SpecialEuclideanOperationTpl<2, _Scalar, _Options> >
{
                    ...
                    const Eigen::MatrixBase<Vector2Like>& t)
    {
...

        {
            ...
            PINOCCHIO_EIGEN_CONST_CAST(Vector2Like, t).coeffRef(0) =
                if_then_else(internal::GT, omega_abs, Scalar(1e-14), vcross.coeff(0), v.coeff(0));

            PINOCCHIO_EIGEN_CONST_CAST(Vector2Like, t).coeffRef(1) =
                if_then_else(internal::GT, omega_abs, Scalar(1e-14), vcross.coeff(1), v.coeff(1));
        }
    }
                    ...
                    const Eigen::MatrixBase<TangentVector>& v)
    {
        ...
        alpha = internal::if_then_else(
            internal::LT, tabs, Scalar(1e-4), 1 - t2 / 12 - t2 * t2 / 720, tabs * st / (2 * (1 - ct)));
                     ...
                     const Eigen::MatrixBase<JacobianOutLike>& J)
    {
        ...
        alpha = internal::if_then_else(internal::LT, tabs, Scalar(1e-4), 1 - t2 / 12, t * st * inv_2_1_ct);
        alpha_dot =
            internal::if_then_else(internal::LT, tabs, Scalar(1e-4), -t / 6 - t2 * t / 180, (st - t) * inv_2_1_ct);
    ...
    static Index nq() { return NQ; }
    /// Get dimension of Lie Group tangent space
    static Index nv() { return NV; }
    ...
    static std::string name() { return std::string("SE(2)"); }
    ...
    : public LieGroupBase<SpecialEuclideanOperationTpl<3, _Scalar, _Options> >
{
    ...
    typedef CartesianProductOperation<VectorSpaceOperationTpl<3, Scalar, Options>,
    ...
    static std::string name() { return std::string("SE(3)"); }
                                ...
                                const Eigen::MatrixBase<Tangent_t>& d)
    {
        ...
        PINOCCHIO_EIGEN_CONST_CAST(Tangent_t, d) = log6(SE3(quat0.matrix(), q0.derived().template head<3>()).inverse() *
                                                        SE3(quat1.matrix(), q1.derived().template head<3>()))
                                                       .toVector();
    }
                          ...
                          const Eigen::MatrixBase<JacobianOut_t>& J) const
    {
        ...
        const SE3 M(SE3(R0, q0.template head<3>()).inverse() * SE3(R1, q1.template head<3>()));
        ...
        if (arg == ARG0)
        {
            ...
            JacobianOut_t& J0 = PINOCCHIO_EIGEN_CONST_CAST(JacobianOut_t, J);
            J0.template bottomRightCorner<3, 3>().noalias() = J0.template topLeftCorner<3, 3>().noalias() =
                               ...
                               const Eigen::MatrixBase<ConfigOut_t>& qout)
    {
        ...
        for (Eigen::DenseIndex k = 0; k < 4; ++k)
        {
            res_quat.coeffs().coeffRef(k) = if_then_else(
                internal::LT, dot_product, Scalar(0), -res_quat.coeffs().coeff(k), res_quat.coeffs().coeff(k));
        }
