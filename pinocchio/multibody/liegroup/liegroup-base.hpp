...
struct LieGroupBase
{
     ...
     *
     * @details    This function computes the Jacobian of the configuration vector variation (component-vise) with
     * respect to a small variation in the tangent.
     *
     * @param[in]  q    configuration vector.
     *
     * @param[out] J    the Jacobian of the log of the Integrate operation w.r.t. the configuration vector q.
     *
     * @remarks    This function might be useful for instance when using google-ceres to compute the Jacobian of the
     * integrate operation.
     */
    ...
    /**
     * @brief      Computes the Jacobian of a small variation of the configuration vector or the tangent vector into
     * tangent space at identity.
     *
     * @details    This Jacobian corresponds to the Jacobian of \f$ (\mathbf{q} \oplus \delta \mathbf{q}) \oplus
     * \mathbf{v} \f$ with \f$ \delta \mathbf{q} \rightarrow 0 \f$ if arg == ARG0 or \f$ \delta \mathbf{v} \rightarrow 0
     * \f$ if arg == ARG1.
     *
     * @param[in]  q    configuration vector.
    ...
    /**
     * @brief      Computes the Jacobian of a small variation of the configuration vector into tangent space at
     * identity.
     *
     * @details    This Jacobian corresponds to the Jacobian of \f$ (\mathbf{q} \oplus \delta \mathbf{q}) \oplus
     * \mathbf{v} \f$ with \f$ \delta \mathbf{q} \rightarrow 0 \f$.
     *
     * @param[in]  q    configuration vector.
     * @param[in]  v    tangent vector.
     * @param[in]  op   assignment operator (SETTO, ADDTO or RMTO).
     ...
     *
     * @details    This Jacobian corresponds to the Jacobian of \f$ \mathbf{q} \oplus (\mathbf{v}  + \delta \mathbf{v})
     * \f$ with \f$ \delta \mathbf{v} \rightarrow 0 \f$.
     *
     * @param[in]  q    configuration vector.
     * @param[in]  v    tangent vector.
     * @param[in]  op   assignment operator (SETTO, ADDTO or RMTO).
     ...
     * respect to the configuration or the velocity arguments.
     *
     * @details This function performs the parallel transportation of an input matrix whose columns are expressed in the
     * tangent space of the integrated element \f$ q \oplus v \f$, to the tangent space at \f$ q \f$. In other words,
     * this functions transforms a tangent vector expressed at \f$ q \oplus v \f$ to a tangent vector expressed at \f$ q
     * \f$, considering that the change of configuration between \f$ q \oplus v \f$ and \f$ q \f$ may alter the value of
     * this tangent vector. A typical example of parallel transportation is the action operated by a rigid
     * transformation \f$ M \in \text{SE}(3)\f$ on a spatial velocity \f$ v \in \text{se}(3)\f$. In the context of
     ...
     * @param[in]  arg    argument with respect to which the differentiation is performed (ARG0 corresponding to q, and
     * ARG1 to v)
     *
     ...
     *
     * @brief   Transport a matrix from the terminal to the originate tangent space of the integrate operation, with
     * respect to the configuration argument.
     *
     * @details This function performs the parallel transportation of an input matrix whose columns are expressed in the
     * tangent space of the integrated element \f$ q \oplus v \f$, to the tangent space at \f$ q \f$. In other words,
     * this functions transforms a tangent vector expressed at \f$ q \oplus v \f$ to a tangent vector expressed at \f$ q
     ...
     *
     * @brief   Transport a matrix from the terminal to the originate tangent space of the integrate operation, with
     * respect to the velocity argument.
     *
     * @details This function performs the parallel transportation of an input matrix whose columns are expressed in the
     * tangent space of the integrated element \f$ q \oplus v \f$, to the tangent space at \f$ q \f$. In other words,
     * this functions transforms a tangent vector expressed at \f$ q \oplus v \f$ to a tangent vector expressed at \f$ q
     ...
     * with respect to the configuration or the velocity arguments.
     *
     * @details This function performs the parallel transportation of an input matrix whose columns are expressed in the
     * tangent space of the integrated element \f$ q \oplus v \f$, to the tangent space at \f$ q \f$. In other words,
     * this functions transforms a tangent vector expressed at \f$ q \oplus v \f$ to a tangent vector expressed at \f$ q
     ...
     *
     * @brief   Transport in place a matrix from the terminal to the originate tangent space of the integrate operation,
     * with respect to the configuration argument.
     *
     * @details This function performs the parallel transportation of an input matrix whose columns are expressed in the
     * tangent space of the integrated element \f$ q \oplus v \f$, to the tangent space at \f$ q \f$. In other words,
     * this functions transforms a tangent vector expressed at \f$ q \oplus v \f$ to a tangent vector expressed at \f$ q
     ...
     *
     * @brief   Transport in place a matrix from the terminal to the originate tangent space of the integrate operation,
     * with respect to the velocity argument.
     *
     * @details This function performs the parallel transportation of an input matrix whose columns are expressed in the
     * tangent space of the integrated element \f$ q \oplus v \f$, to the tangent space at \f$ q \f$. In other words,
     * this functions transforms a tangent vector expressed at \f$ q \oplus v \f$ to a tangent vector expressed at \f$ q
     ...
     *
     * \cheatsheet \f$ q_1 \equiv  q_0 \oplus \left( q_1 \ominus q_0 \right) \f$ (\f$\equiv\f$ means equivalent, not
     * equal).
     */
    template <class ConfigL_t, class ConfigR_t>
    ...
    bool operator==(const LieGroupBase& other) const { return derived().isEqual_impl(other.derived()); }

    bool operator!=(const LieGroupBase& other) const { return derived().isDifferent_impl(other.derived()); }
    /// \}
    ...
    template <class ConfigL_t, class ConfigR_t>
    ConfigVector_t randomConfiguration(const Eigen::MatrixBase<ConfigL_t>& lower_pos_limit,
    ...
    bool isDifferent_impl(const LieGroupBase& other) const { return !derived().isEqual_impl(other.derived()); }
