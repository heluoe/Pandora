    ...
    : public LieGroupBase<SpecialOrthogonalOperationTpl<2, _Scalar, _Options> >
{
    ...
    static typename Matrix2Like::Scalar log(const Eigen::MatrixBase<Matrix2Like>& R)
    {
        ...
        Scalar theta =
            if_then_else(internal::GT,
                         tr,
                         Scalar(2),
                         Scalar(0),  // then
                         if_then_else(internal::LT,
                                      tr,
                                      Scalar(-2),
                                      if_then_else(internal::GE, R(1, 0), Scalar(0), PI_value, -PI_value),  // then
                                      if_then_else(internal::GT,
                                                   tr,
                                                   Scalar(2) - 1e-2,
                                                   asin((R(1, 0) - R(0, 1)) / Scalar(2)),  // then
                                                   if_then_else(internal::GE,
                                                                R(1, 0),
                                                                Scalar(0),
    ...
    static std::string name() { return std::string("SO(2)"); }
                                 ...
                                 const Eigen::MatrixBase<ConfigOut_t>& qout)
    {
        ...
        {
            out = (sin((1 - u) * theta) / sinTheta) * q0 + (sin(u * theta) / sinTheta) * q1;
        }
    ...
    : public LieGroupBase<SpecialOrthogonalOperationTpl<3, _Scalar, _Options> >
{
    ...
    static std::string name() { return std::string("SO(3)"); }
                                ...
                                const Eigen::MatrixBase<Tangent_t>& d)
    {
        ...
        PINOCCHIO_EIGEN_CONST_CAST(Tangent_t, d) = log3((quat0.matrix().transpose() * quat1.matrix()).eval());
    }
                          ...
                          const Eigen::MatrixBase<JacobianOut_t>& J) const
    {
        ...
        const Matrix3 R =
            quat0.matrix().transpose() *
            quat1.matrix();  // TODO: perform first the Quaternion multiplications and then return a Rotation Matrix
